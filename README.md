...
- id: dsh-desktop
  config:
    ...
    shortcut:
      ...
      icon: mascot        # 图标名（包内 mascot.ico），或任意 .ico 的绝对路径
...
- **Linux（GNOME/KDE）**：在 `~/.local/share/applications/` 放一个 `.desktop` 文件，`Exec=dsh web`，`Icon` 可指向 `mascot.ico`（如需 png，可用 ImageMagick 转换）


## 图标版权

- 包内只附带一个图标 `mascot.ico`：仓库维护者自有素材，无额外限制。
- 代码部分：MIT
- 想换图标：把 `shortcut.icon` 指向你自己的 `.ico`，或直接替换包内的 `mascot.ico`。
