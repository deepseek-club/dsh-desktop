/**
 * dsh-web-launcher v0.2.0 鈥?one-click launcher plugin for the DeepSeek
 * Harness Web GUI.
 *
 * Two features:
 ...
 *     pointing at it 鈥?so every install gets the pretty one-click launcher
 ...
 *   enabled:      boolean, default true          鈥?master switch
 *   delayMs:      number,  default 500           鈥?open delay after service ready
 *   url:          string,  default derived from the bound webServer port
 *   shortcut:     object  鈥?desktop shortcut feature
 ...
 *     icon:         string,  default mascot (or absolute path)
 *     description:  string,  default DSH Desktop 鈥?涓€閿惎鍔?DeepSeek Harness
 */
...
export function resolveDesktopDir() {
  ...
  for (const cand of [join(HOME, 'Desktop'), join(HOME, 'OneDrive', 'Desktop'), join(HOME, 'OneDrive', '妗岄潰')]) {
 ...
 * @param {string} o.icon - absolute .ico path
  ...
  if (shortcutCfg.enabled !== false && process.platform === 'win32') {
    try {
      const info = installShortcut({
        ...
        description: shortcutCfg.description ?? 'DSH Desktop 鈥?涓€閿惎鍔?DeepSeek Harness',
      });
