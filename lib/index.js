 ...
 *     shortcutName: string,  default 启动 DeepSeekHarness
 *     icon:         string,  default whale-maid (or whale-shield / absolute path)
 *     description:  string,  default 一键启动 DeepSeek Harness Web 界面（鲸鱼娘版）
 */
...
export const name = 'web-launcher';
  ...
  if (shortcutCfg.enabled !== false && process.platform === 'win32') {
    try {
      const info = installShortcut({
        ...
        shortcutName: shortcutCfg.shortcutName ?? '启动 DeepSeekHarness',
        icon: shortcutCfg.icon ?? 'whale-maid',
        description: shortcutCfg.description ?? '一键启动 DeepSeek Harness Web 界面（鲸鱼娘版）',
      });
